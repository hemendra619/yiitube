 ...
 * @author Hemendra Chaudhary <hemendra.chaudhary619@gmail.com>
 */
class Yiitube extends CWidget {
    ...
    public function init() {
        ...
        if ($this->v != NULL) {
            //echo "here v not null";exit;
            if (filter_var($this->v, FILTER_VALIDATE_URL)){
                //echo "here url validated";exit;
                if(strpos($this->v, 'youtu.be')){
                   $this->iframe=true;
                }
                $this->v = $this->getVideoCode();
            }else{
                if(strpos($this->v, 'iframe') || strpos($this->v, 'youtu.be')){
                   $this->iframe=true;
                }
            }
        }
...
VEOH;
    ...
    protected function youtubeVideoURL() {
        if ($this->iframe) {
            ...
            preg_match('#(\.be/|/embed/|/v/|/watch\?v=)([A-Za-z0-9_-]{5,11})#', $this->v, $match);
            $id = $match[2];
    ...
    protected function renderError() {
        switch ($this->_status) {
            case self::NO_VIDEO_CODE:
                return Yii::t('yiitube', 'you need to provide a video code to display video<br/>');
                break;
            case self::NO_VIDEO_CODE_IN_URL:
                return Yii::t('yiitube', 'the URL you provided does not contain a valid video code.');
                break;
            case self::NO_SUPPORTED_PLAYER:
                return Yii::t('yiitube', '{player} is not supported.', array('{player}' => $this->player));
                break;
        }
    }
